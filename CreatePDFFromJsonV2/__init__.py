...
def main(req: HttpRequest) -> HttpResponse:
    #try:
        ...
        #Attributten sier noe om hva slags type Json data dette er. Er det for konsultasjon, resept etc:
        jsonDataType = data.get("jsonDataType")

        #henter Templatename:
        template_url = get_template_url(jsonDataType, clinic_data)

        #Sjekker om det er en resept som mottas, og henter signatur
        if jsonDataType == "prescription":
            signature_raw = data.get("VetSignature",{})
            logging.info(signature_raw)
            # Finn startindeksen for base64-dataen
            start_index = signature_raw.find("base64,") + len("base64,")
            # Ekstraher base64-dataen
            base64_data = signature_raw[start_index:]
            # Dekode base64-dataen til binær data
            binary_data = base64.b64decode(base64_data)
            signature_base64 = base64.b64encode(binary_data).decode('utf-8')


            data['signature_base64'] = signature_base64
            #data['signature_base64_raw'] = signature_raw
...
def get_template_url(jsonDataType, clinic_data):
    ...
    if jsonDataType == "consultation":
        ...
        return consultationMerge_folder + "template_3.html"
    else:
        return consultation_folder + clinic_data.ConsultationTemplateName